...
// MasterChef is the master of Bami. He can make BAMI and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once BAMI is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    function pendingBami(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 bamiReward = multiplier.mul(bamiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBamiPerShare = accBamiPerShare.add(bamiReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 bamiReward = multiplier.mul(bamiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        bami.mint(devaddr, bamiReward.div(10));
        bami.mint(address(this), bamiReward);
        pool.accBamiPerShare = pool.accBamiPerShare.add(bamiReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBamiPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeBamiTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
